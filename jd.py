...
def muchos_train_test_resampled(X,y,modelo,n=20, resampling="SMOTETomek",seed=123,title=False):
    """
    ...
    if resampling== "SMOTETomek":
        resampler = SMOTETomek(random_state=rng.integers(1000000))
    elif resampling== "RandomUnderSampler":
        resampler = RandomUnderSampler(random_state=rng.integers(1000000))
    elif resampling== "ADASYN":
        resampler = ADASYN(random_state=rng.integers(1000000))
    elif resampling== "SMOTE":
        resampler = SMOTE(random_state=rng.integers(1000000))
    elif resampling== "BorderlineSMOTE":
        resampler = BorderlineSMOTE(random_state=rng.integers(1000000))
    elif resampling== "SVMSMOTE":
        resampler = SVMSMOTE(random_state=rng.integers(1000000))
    elif resampling== "KMeansSMOTE":
        resampler = KMeansSMOTE(random_state=rng.integers(1000000))
...
def grafica_relacion_MissingsTiempo(df,col_name,columna_temporal = "created_month",titulo = "",return_dfConColMissing = False):
    
    #df_analiza_Falta = df[df["target"]!=2].copy()
    col_name = str(col_name) +"_missing"
    df_analiza_Falta[col_name] = df[col_name].apply(lambda x: 1 if pd.isna(x) else 0)
    if return_dfConColMissing:
            df_analiza_Falta = df_analiza_Falta[[col_name,columna_temporal]]
            to_return = df_analiza_Falta.copy()
            df_analiza_Falta = df_analiza_Falta.groupby(columna_temporal).mean()
